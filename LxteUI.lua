...
for Index, Button in next, Sidebar.Top.Buttons:GetChildren() do
	if Button:IsA("TextButton") then
		...
		Button.Text = Symbols[Name] or Name
	...
	function Options:AddDropdown(Settings: { Title: string, Description: string, Options: {}, Tab: Instance, Callback: any })
		...
		local IsMulti = Settings.Multi == true;
...
for Index, Option in next, Settings.Options do
	...
	if IsMulti then
		...
		if Selected.Value then
			...
			table.insert(SelectedOptions, Index)
		else
		...
		Text.Text = table.concat(SelectedOptions, ", ")
		Settings.Callback(SelectedOptions)
	else
		-- SINGLE-SELECT MODE
		for _, otherBtn in ipairs(Example.ScrollingFrame:GetChildren()) do
			if otherBtn:IsA("TextButton") and otherBtn ~= Button then
				otherBtn.BackgroundColor3 = Theme.Component
				otherBtn.Value.Value = false
			end
		end
